...
void CCETxnInputGenerator::Initialize()
{
    ...
    m_iActiveSecurityPerServer = m_iActiveSecurityCount / total_partition;
    assert(m_iActiveSecurityPerServer * total_partition == m_iActiveSecurityCount);
    m_iStartSecurity = m_iActiveSecurityPerServer * current_partition;
...
void CCETxnInputGenerator::GenerateBrokerVolumeInput(TBrokerVolumeTxnInput1 &TxnReq) {
  INT32           iNumBrokers;
  INT32           iCount, i;
  ...
  do
    {
      ...
      TxnReq.broker_list[iCount] = m_Brokers.GenerateRandomBrokerId(&m_rnd);
      
      for (i = 0; (i < iCount) && (TxnReq.broker_list[i] != TxnReq.broker_list[iCount]); ++i) { };
      if(i == iCount) iCount++;
...
void CCETxnInputGenerator::GenerateTradeUpdateInput(TTradeUpdateTxnInput &TxnReq)
{
	...
	//	fprintf(stdout,"test val %lu, %lu\n",TradeUpdateAValueForSymbolFrame3,TradeUpdateSValueForSymbolFrame3);
	//	auto seed = m_rnd.GetSeed();
	//	auto test = m_rnd.RndInt64Range(0,m_iActiveSecurityCount - 1);
	//	m_rnd.SetSeed(seed);
	//	auto test2 = m_rnd.NURnd(0,m_iActiveSecurityCount-1,TradeUpdateAValueForSymbolFrame3,
	//				 TradeUpdateSValueForSymbolFrame3);
	//	assert(test == test2);
	//	fprintf(stdout,"test val %lu, test val2 %lu\n",test,test2);

	/* FIXME: i change here so that the symbol will be all local 
	   Since the security is generated uniformly since A,S value are all zero. 
	   So symbol can be generated locally with RndInt64Range. 
	 */
	//        m_pSecurities->CreateSymbol( m_rnd.NURnd( 0, m_iActiveSecurityCount-1,
	...
	//                                    TxnReq.symbol,
	//                                    static_cast<int>(sizeof( TxnReq.symbol )));
	m_pSecurities->CreateSymbol(m_rnd.RndIntRange(m_iStartSecurity,
						   m_iStartSecurity + m_iActiveSecurityPerServer - 1),
