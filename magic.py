...
def configure_deps_dir(s):
    ...
    cmake_file.write("""cmake_minimum_required(VERSION 3.2)\ninclude(ExternalProject)\n""")
...
""" % (k,k,k,k,deps[k]["include_dir"])
            ...
            deps_list.append(k)
            dep = deps[k]
            cmake_file.write("""set(%s_INSTALL_DIR ${CMAKE_SOURCE_DIR}/deps/%s)\n""" % (k,k))
            ...
            ## add include dir
            cmake_file.write("""include_directories(BEFORE ${%s_INSTALL_DIR}/include)""" % k)
...
def write_header(f):
    ...
    f.write("project(xxx)\n")
    f.write("ADD_DEFINITIONS(-std=c++14)\n")
...
def load_downloads(s):
    try:
        ...
        for name in downs:
            i = downs[name]
            if ".git" in i["url"]:
                call(["git","submodule","add",i["url"],"deps/%s" % name,"--force"])
            else:
...
def main():
    ...
    parser_new = subparsers.add_parser('config', help='configure the whole projects')
