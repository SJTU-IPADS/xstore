...
(either using `git submodule` or `cmake external project`).
There are two exceptions: **Intel MKL** and **Mellanox OFED**.
Below, we first briefly describe how to install them. 

- **Intel MKL**
There are two ways to install Intel MKL.
One way is to download the install package from its official website. 
For example, we downloaded a tar file named `l_mkl_2020.3.279`.
After untar it, we run the `./install.sh` and follow its instructions to complete the installation.
Note that the install process may depend on cpio, which can be installed by `apt-get update; apt-get install cpio`.
Another way is to install it from `apt`.
Please follow the instructions at [](https://software.intel.com/content/www/us/en/develop/articles/installing-intel-free-libs-and-python-apt-repo.html) for the detailed information.
...
- **Mellanox OFED**
After downloading the specific OFED from [OFED](https://www.mellanox.com/products/infiniband-drivers/linux/mlnx_ofed),
we use `mlnxofedinstall --without-iser-dkms --without-srp-dkms --without-srptools --force` to install it.
