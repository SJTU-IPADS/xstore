...
// gflags string is conflict w pytorch, so currently xstore does not support
// runnign NN with gflag string DEFINE_string(addr, "localhost:8888", "server
// address");
...
fill_tpcc_keys()
{
  ...
  while (getline(myfile, line)) {
    ...
    auto key = KK(w, d, o, c).d;
    all_keys->push_back(key);

    count += 1;
    if (count >= FLAGS_nkeys) {
      return;
    }
  }
}
  ...
  for (uint thread_id = 0; thread_id < FLAGS_threads; ++thread_id) {
    workers.push_back(std::move(
      std::make_unique<XThread>([&statics, &bar, thread_id]() -> usize {
        // bind the core
        //::xstore::platforms::VALBinder::bind(thread_id % 2, thread_id);
        ...
        UDTransport sender;
        {
          ...
          do {
            auto res = sender.connect(
              server_addr, "b" + std::to_string(thread_id), id, qp);
            if (res == IOCode::Ok) {
              break;
            }
            if (t.passed_sec() >= 10) {
        ...
        if (thread_id == 0) {
...

          {
            ...
            cur_ptr = xcache_buf + (meta.tt_buf - meta.model_buf);
