...
extern thread_local char* rpc_large_reply_buf;
extern thread_local u32 rpc_large_reply_key;

void
meta_callback(const Header& rpc_header, const MemBlock& args, SendTrait* replyc)
{
  ...
  std::string dispatcher = "";
  {
    if (std::is_same<XCache::FirstML<KK>, TNN<KK>>()) {
      ...
      dispatcher = cache->first_layer.serialize();
    }
  }
  ...
  ASSERT(sizeof(ReplyMeta) + dispatcher.size() + sizeof(Header) <= 256)
    << sizeof(ReplyMeta) << " " << dispatcher.size();

  // send
  RPCOp op;
  ASSERT(
    op.set_msg(MemBlock(rpc_large_reply_buf, 1024)).set_reply().add_arg(meta));
  ASSERT(op.add_opaque(dispatcher));
  op.set_corid(rpc_header.cor_id);

  auto ret = op.execute_w_key(replyc, rpc_large_reply_key);
 ...
 */
void
get_callback(const Header& rpc_header, const MemBlock& args, SendTrait* replyc)
{
  // sanity check the requests
  ASSERT(args.sz == sizeof(u64));
  ...
  // get()
  auto value = db.get(KK(key)).value();
