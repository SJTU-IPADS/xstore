...
template <usize N, typename K> struct TreeInner {
  ...
  TreeInner() : up_key(kInvalidKey) {
    // zeroing
    std::fill_n(childrens, N, nullptr);
  }

  TreeInner(const K &k, const raw_ptr_t &l, const raw_ptr_t &r) : num_keys(1) {
    this->keys[0] = k;
    this->childrens[0] = l;
    this->childrens[1] = r;
    this->num_keys = 1;
  ...
  template <typename V>
  TreeInner<N, K> *insert(const K &key, const V &v, int depth,
                          bool &whether_split, XNode<N, K, V> *candidate) {
    ASSERT(depth >= 1);
